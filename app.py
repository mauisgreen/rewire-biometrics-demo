...
if st.session_state.get("assessment_done"):
    ...
    # Initialize per‐patient defaults exactly once
    ...
    # If patient changes, clean previous form keys to prevent duplicates
    if st.session_state.get("last_pid") != pid:
        keys_to_clear = [k for k in st.session_state.keys() if any(k.startswith(p) for p in [
            "g1_", "g2_", "g3_", "f1_", "f2_", "f3_", "note_"
        ])]
    ...
    # Unique selectboxes and sliders per patient
    with st.form(key=f"plan_form_{pid}", clear_on_submit=False):
        ...
        f1 = st.slider(f"{g1} per week", 1, 7, defaults["freqs"][0], key=f"f1_{pid}_{g1}")
        f2 = st.slider(f"{g2} per week", 1, 7, defaults["freqs"][1], key=f"f2_{pid}_{g2}")
        f3 = st.slider(f"{g3} per week", 1, 7, defaults["freqs"][2], key=f"f3_{pid}_{g3}")

        note = st.text_area("Message to patient", value=defaults["note"], key=f"note_{pid}")
